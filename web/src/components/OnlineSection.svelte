<script>
  ...
  import { onlineStatus, onlineConnect, onlineDisconnect, onlineAcceptDisclaimer, onlinePublish, onlineSync, onlineNeighbors, onlinePreview, onlineUnpublish } from '../lib/api.js';
  ...
  let lastSyncAt = null;
  let syncing = false;
  let preview = null; // „Was wird veröffentlicht?" im Disclaimer
  ...
  async function openDisclaimer() {
    showDisclaimer = true;
    preview = null;
    try { const p = await onlinePreview(); if (p.ok) preview = p; } catch {}
  }
  async function goOffline() {
    if (busy) return; busy = true;
    try {
      const r = await onlineUnpublish();
      publishMsg = r.ok ? { ok: true, text: `✓ Offline gegangen — ${r.deleted} Datei(en) werden aus dem Repo entfernt.` } : { ok: false, text: r.error };
      await refresh();
    } catch (e) { publishMsg = { ok: false, text: e.message }; }
    busy = false;
  }
...
<div class="mt-2.5 pt-2 border-t border-stone-800">
  ...
  {:else if st.connected}
    ...
    {#if st.disclaimerAccepted && st.publishEnabled}
      ...
      {/if}
      <button class="mt-1 w-full text-[11px] text-stone-500 hover:text-red-300" on:click={goOffline} disabled={busy} title="Eigene Daten aus dem Community-Repo entfernen; nächtliche Veröffentlichung stoppt">
        🚪 Offline gehen (Daten aus dem Repo entfernen)
      </button>
    {:else}
      <button class="mt-1.5 w-full rounded bg-emerald-800 hover:bg-emerald-700 disabled:opacity-50 px-3 py-1.5 text-sm text-white" on:click={openDisclaimer} disabled={busy}>
...
{#if showDisclaimer}
  <div class="fixed inset-0 z-[110] bg-black/70 grid place-items-center p-4">
    <div class="w-full max-w-lg max-h-[85vh] overflow-y-auto rounded-lg border border-amber-700 bg-stone-900 shadow-2xl p-4">
      ...
      {#if preview}
        <div class="mt-2 rounded border border-stone-700 bg-stone-950/60 px-2 py-1.5 text-[11px] text-stone-400">
          <b class="text-stone-300">Das wird veröffentlicht:</b> Karte {preview.mapSize} · {preview.instances} Gebäude · {preview.roads} Straßenfelder ·
          {preview.packBuildings} KI-Gebäude & {preview.packResources} KI-Ressourcen (Baupläne) · GitHub-Name „{preview.username}"
        </div>
      {/if}
