...
export async function loadWorld(pool) {
  const res = await pool.query('SELECT seed, width, height, tiles, islands, version FROM world WHERE id = 1');
  if (res.rowCount === 0) return null;
  const r = res.rows[0];
  return { seed: Number(r.seed), width: r.width, height: r.height, tiles: r.tiles, islands: r.islands || [], version: r.version || 0 };
}

export async function saveWorld(pool, world) {
  await pool.query(
    `INSERT INTO world (id, seed, width, height, tiles, islands, version)
    ...
    [world.seed, world.width, world.height, world.tiles, JSON.stringify(world.islands || []), world.version || 0]
  );
}
